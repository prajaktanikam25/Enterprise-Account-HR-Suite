...
import { EXCEL_FILE_INCOME_BASE64 } from '../../../utils/import-income';
import FileSaver from 'file-saver';

const ImportIncome = ({ handleClose }) => {
    const [selectedFile, setSelectedFile] = useState();

    const postData = () => {
        const formData = new FormData()
        formData.append("excel-file", selectedFile)
        axios.post(BASE_URL + "/import", formData, { headers: { "Content-Type": "multipart/form-data" } });
        console.log(selectedFile);
        handleClose();
    }
    const handleImport = () => {
        let sliceSize = 1024;
        let byteCharacters = atob(EXCEL_FILE_INCOME_BASE64);
