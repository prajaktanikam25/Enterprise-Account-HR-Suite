...
const AddIncome = ({ theEditData, handleClose }) => {
    const [organization, setOrganization] = useState([]);
    const [department, setDepartment] = useState([]);
    const [companyName, setCompanyName] = useState(theEditData.companyName);
    const [purpose, setPurpose] = useState(theEditData.purpose);
    const [clientName, setClientName] = useState(theEditData.clientName);
    const [paymentMode, setPaymentMode] = useState(theEditData.paymentMode);
    // const [commission, setCommission] = useState([]);
    const [invoiceNo, setInvoiceNo] = useState(theEditData.invoiceNo);
    const [gst, setGst] = useState();
    const [amount, setAmount] = useState(theEditData.amount);
    const [totalAmount, setTotalAmount] = useState(theEditData.totalAmount);
    const [employee, setEmployee] = useState([]);
    const [date, setDate] = useState(theEditData.date);
    const [tick, setTick] = useState(theEditData.gst);
    ...
    // const [myOptions5, setMyOptions5] = useState(theEditData.commission.name);
    const [myOptions6, setMyOptions6] = useState(theEditData.employee);
    ...
    const gstamt = (tick * totalAmount) / 100;
    // console.log(gstamt)
    ...
    const postData = () => {
        ...
        const IncomeData = {
            ...
            employee: theEditData.employee,
            date: date,
            status: status,
            isDeleted: theEditData.isDeleted
        };
        axios.put(BASE_URL + '/updateIncome/' + theEditData._id, IncomeData)
